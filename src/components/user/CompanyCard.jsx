...
const CompanyCard = ({
  ...
  const overdue =
    notifications.overdue?.some((comp) => comp._id === company._id) || false;
  const dueToday =
    notifications.dueToday?.some((comp) => comp._id === company._id) || false;

  const highlightClass =
    !highlightDisabled[company._id] && overdue
  ...
  return (
    <div
      className={`p-6 rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-300 ${highlightClass}`}
    >
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <input
                ...
                className="h-6 w-6 rounded-full text-blue-600 focus:ring-blue-500 cursor-pointer"
              />
              <h3 className="text-3xl font-semibold text-gray-900 hover:text-blue-700 transition-colors">
                {company.name}
              </h3>
            </div>
            <button
              className="text-sm px-4 py-2 bg-gradient-to-r from-indigo-500 to-blue-600 text-white rounded-lg hover:bg-gradient-to-l transition-all"
        ...
        <div className="bg-gradient-to-br from-gray-50 to-white rounded-lg p-5 hover:bg-gradient-to-bl transition-all duration-300">
            ...
            {lastFiveDates.length > 0 ? (
              lastFiveDates.map((communication) => (
                <li
                  ...
                  className="text-sm bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 relative cursor-pointer group"
                ...
                >
                  ...
                  {hoveredCommunication === communication &&
                    communication.notes && (
                      <div className="absolute z-20 p-5 bg-gray-800 text-white text-sm rounded-lg shadow-xl mt-3 max-w-xs transform -translate-x-1/2 left-1/2">
            ...
            <div className="space-y-5">
              <div className="flex justify-between items-start">
                <div className="space-y-2">
                  {activeSchedules[company._id] ? (
                    <>
                      <span className="px-3 py-2 bg-blue-200 text-blue-800 rounded-full text-sm font-medium">
                ...
                <div className="flex gap-3">
                  {activeSchedules[company._id] && (
                    <button
                      ...
                      className="px-4 py-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 text-sm font-medium transition-all duration-300"
                    >
                      Cancel
                    </button>
                  )}
                  <button
                    ...
                    className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium transition-all duration-300"
                  >
              ...
              <div className="flex items-center gap-3 pt-4 border-t">
                <input
                  ...
                  className="h-5 w-5 rounded-full text-blue-600 focus:ring-blue-500 cursor-pointer"
                />
                <label
