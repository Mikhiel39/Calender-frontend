...
export const AppProvider = ({ children }) => {
    ...
    const fetchInitialData = async () => {
      try {
        const [companiesRes, methodsRes, schedulesRes] = await Promise.all([
          ...
          axios.get("https://calender-backend-i1bj.onrender.com/api/communication-methods"),
          axios.get("https://calender-backend-i1bj.onrender.com/api/next-communications"),
        ]);
  ...
  const addCompany = async (newCompany) => {
    try {
      ...
      await axios.post("https://calender-backend-i1bj.onrender.com/api/companies", newCompany);
      const response = await axios.get("https://calender-backend-i1bj.onrender.com/api/companies");
  ...
  const deleteCompany = async (id) => {
    try {
      await axios.delete(`https://calender-backend-i1bj.onrender.com/api/companies/${id}`);
  ...
  const addMethod = async (newMethod) => {
    try {
      await axios.post(
        "https://calender-backend-i1bj.onrender.com/api/communication-methods",
        newMethod
      );
  ...
  const updateMethod = async (updatedMethod) => {
    try {
      await axios.put(
        `https://calender-backend-i1bj.onrender.com/api/communication-methods/${updatedMethod._id}`,
        updatedMethod
      );
      setMethods((prev) =>
        prev.map((method) =>
  ...
  const deleteMethod = async (id) => {
    try {
      await axios.delete(
        `https://calender-backend-i1bj.onrender.com/api/communication-methods/${id}`
      );
...
const cancelNextCommunication = async (companyId) => {
  try {
    if (activeSchedules[companyId]) {
      await axios.delete(`https://calender-backend-i1bj.onrender.com/api/next-communications/${activeSchedules[companyId]._id}`);
      setActiveSchedules((prev) => {
...
const removeCommunication = async (companyId, communicationId) => {
  try {
    await axios.delete(`https://calender-backend-i1bj.onrender.com/api/communications/${communicationId}`);
